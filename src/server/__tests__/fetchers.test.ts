 ...
 *
 * Also tests email-vs-ID detection in fetchSubscription and the stub
...
describe('server fetchers', () => {
  ...
  describe('fetchWebhooks', () => {
    it('should throw an error indicating the endpoint is not available', async () => {
      await expect(fetchWebhooks(client, 'pub_abc')).rejects.toThrow(
        /WebhooksEndpoint not yet available/,
      );
    });
  });
  ...
  describe('fetchSegments', () => {
    it('should throw an error indicating the endpoint is not available', async () => {
      await expect(fetchSegments(client, 'pub_abc')).rejects.toThrow(
        /SegmentsEndpoint not yet available/,
      );
    });
  });
});
